...
st.set_page_config(page_title="UMAP Exploration", layout="wide")
st.title("Page 3: UMAP Exploration")
...
if "shared_df" in st.session_state:
    df_uploaded = st.session_state["shared_df"]
    uploaded_file_original = df_uploaded.copy()
    # Only proceed if a file is uploaded
    if uploaded_file_original is not None:
        original_data = uploaded_file_original
    ...
    cat_cols = ["primary surfactant name", "secondary surfactant name", "tertiary surfactant name", 
                "clarity", "colour", "physical state", "cmc"]
        ...
        if not df1_filtered.empty:
            ...
            if color_feature == "Type":
                fig1 = px.scatter(
                    ...
                    y="UMAP2",
                    color="Type",
                    hover_data= ["Index"] + interactive_df1_columns,
        ...
        if not df2_filtered.empty:
            ...
            if color_performance == "Type":
                fig2 = px.scatter(
                    ...
                    y="UMAP2",
                    color="Type",
                    hover_data=["Index"] + interactive_df2_columns,
                    color_discrete_map={"Original": "#4A90E2", "Synthetic": "#E74C3C", "New": "#553500"}
                )
        ...
        if not df3_filtered.empty:
            ...
            if color_feature_performance == "Type":
                fig3 = px.scatter(
                    ...
                    y="UMAP2",
                    color="Type",
                    hover_data=["Index"] + interactive_df3_columns,
                    color_continuous_scale="plasma",
                    color_discrete_map={"Original": "#4A90E2", "Synthetic": "#E74C3C", "New": "#553500"}
                )
    ...
    with st.expander("UMAP: Custom Selected Variables", expanded=False):
            ...
            if not df4_filtered.empty:
                ...
                if color_custom == "Type":
                    fig4 = px.scatter(
                        ...
                        y="UMAP2",
                        color="Type",
                        hover_data=["Index"] + selected_cols,
                        color_discrete_map={"Original": "#4A90E2", "Synthetic": "#E74C3C", "New": "#553500"}
                    )
    ...
    #-- Surfactant Blend Price Calculator ---
    # Price ranges (per kg in £)
    price_dict = {
        "ADINOL CT24": (2.8, 4.0),
        "ADINOL CT30DS": (3.5, 4.5),
        "CRODASINIC CS30": (4.5, 6.0),
        "CRODASINIC LS30": (3.7, 5.7),
        "CRODASINIC MS30": (5.6, 8.0),
        "Pureact": (15, 35),
        "SLES": (0.9, 1.5),
        "CRODATERIC CAB30-LQ-(TH)": (3.5, 5.5),
        "Decyi Glucoside": (8.0, 10.0)
    }

    primary_options = [
        "ADINOL CT24", "ADINOL CT30DS", "CRODASINIC CS30",
        "CRODASINIC LS30", "CRODASINIC MS30", "Pureact", "SLES"
    ]
    secondary_options = ["CRODATERIC CAB30-LQ-(TH)"]
    tertiary_options = ["Decyi Glucoside"]

    with st.expander("💰 Surfactant Blend Price Calculator", expanded=False):
        blend_weight = st.number_input("Enter Blend Weight (g)", min_value=1.0)

        st.markdown("### 🧪 Primary Surfactant")
        col1, col2, col3 = st.columns(3)
        with col1:
            primary_name = st.selectbox("Primary Surfactant", primary_options, key="prim")
        with col2:
            primary_active = st.number_input("Primary Active (%)", min_value=0.01, key="prim_act")
        with col3:
            primary_level = st.number_input("Primary Level (%)", min_value=0.01, key="prim_lvl")

        show_secondary = st.radio("Do you want to add a secondary surfactant?", ["No", "Yes"], horizontal=True)

        if show_secondary == "Yes":
            st.markdown("### 🧪 Secondary Surfactant")
            col4, col5, col6 = st.columns(3)
            with col4:
                secondary_name = st.selectbox("Secondary Surfactant", secondary_options, key="sec")
            with col5:
                secondary_active = st.number_input("Secondary Active (%)", min_value=0.01, key="sec_act")
            with col6:
                secondary_level = st.number_input("Secondary Level (%)", min_value=0.01, key="sec_lvl")

            show_tertiary = st.radio("Do you want to add a tertiary surfactant?", ["No", "Yes"], horizontal=True)

            if show_tertiary == "Yes":
                st.markdown("### 🧪 Tertiary Surfactant")
                col7, col8, col9 = st.columns(3)
                with col7:
                    tertiary_name = st.selectbox("Tertiary Surfactant", tertiary_options, key="tert")
                with col8:
                    tertiary_active = st.number_input("Tertiary Active (%)", min_value=0.01, key="tert_act")
                with col9:
                    tertiary_level = st.number_input("Tertiary Level (%)", min_value=0.01, key="tert_lvl")
        ...
        if st.button("🧮 Calculate Price Range"):
            ...
            def compute_cost(level_pct, active_pct, price_min, price_max):
                weight = blend_weight * (level_pct / active_pct)
                cost_min = (weight / 1000) * price_min
                cost_max = (weight / 1000) * price_max
                return cost_min, cost_max

            # Primary
            pmin, pmax = price_dict[primary_name]
            cmin, cmax = compute_cost(primary_level, primary_active, pmin, pmax)
            total_min_price += cmin
            total_max_price += cmax

            # Secondary
            if show_secondary == "Yes":
                smin, smax = price_dict[secondary_name]
                cmin, cmax = compute_cost(secondary_level, secondary_active, smin, smax)
                total_min_price += cmin
                total_max_price += cmax

                # Tertiary
                if show_tertiary == "Yes":
                    tmin, tmax = price_dict[tertiary_name]
                    cmin, cmax = compute_cost(tertiary_level, tertiary_active, tmin, tmax)
                    total_min_price += cmin
                    total_max_price += cmax

            st.success(f"💸 Estimated Blend Price Range: **£{total_min_price:.2f} - £{total_max_price:.2f}**")


else:
    st.warning("⚠️ Please upload an Excel file on the Upload Data page to proceed with UMAP exploration.")